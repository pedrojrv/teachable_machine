...
async function train() { // TRAINS A MODEL USING WEBCAM COLLECTED IMAGES AND CREATES VISOR DATA
  ...
  model = tf.sequential({
    layers: [
      tf.layers.cropping2D({cropping:[[70, 70], [70, 70]], inputShape: [224, 224, 3]}),
      tf.layers.conv2d({kernelSize: 2, filters: 2, activation: 'relu', kernelInitializer:'varianceScaling'}),
...
async function add_data(controllerDataset, label) { // ADS SAVED WEBCAM IMAGE TO A GIVEN DATASET
  ...
  controllerDataset.addExample(img, label);
  // img.dispose();
};
...
// ############################################################
// ############################################################
async function init(){// CALLS FOR WEBCAM INSTANCE
  try { 
...
function open_survey() { // OPENS ML AT APPLE SURVEY
  var survey_frame = document.getElementById("ml_apple_survey");
  survey_frame.innerHTML = "<iframe src=\"https://essentials.applesurveys.com/jfe/form/SV_8phOxggY2dGoe45\" class=\"col-12\" height=\"600px\"></iframe>";
  survey_frame.style.marginBottom = "5%";
};